...
static void print_help(void)
{
    cerr << "\nvideo_decode <in-format> [options] <in-file>\n\n"
            ...
            "\tNOTE: this should not be used alongside -o option as it "
            "decreases the FPS value shown in --stats\n"
            ...
            "\t-ww <width>          Window width in pixels [Default = "
            "video-width]\n"
            "\t-wh <height>         Window height in pixels [Default = "
            "video-height]\n"
            "\t-loop <count>        Playback in a loop.[count = 1,2,...,n "
            "times looping , 0 = infinite looping]\n"
            "\t-queue [<file1> <file2> ....] Files are played in a queue "
            "manner\n"
            "\tNOTE: -queue should be the last option mentioned in the command "
            "line no other option should be mentioned after that.\n"
            "\t-wx <x-offset>       Horizontal window offset [Default = 0]\n"
            "\t-wy <y-offset>       Vertical window offset [Default = 0]\n\n"
            "\t-fps <fps>           Display rate in frames per second [Default "
            "= 30]\n\n"
            ...
            "\t--input-chunks       Input to the decoder will be a chunk of "
            "bytes [Default]\n\n"
            "\t--copy-timestamp <st> <fps> Enable copy timestamp with start "
            "timestamp(st) in seconds for decode fps(fps) (for input-nalu "
            "mode)\n"
            "\tNOTE: copy-timestamp used to demonstrate how timestamp can be "
            "associated with an individual H264/H265 frame to achieve "
            "video-synchronization.\n"
            "\t      currenly only supported for H264 & H265 video encode "
            "using MM APIs and is only for demonstration purpose.\n"
            "\t--report-metadata    Enable metadata reporting\n\n"
            "\t--blocking-mode <val> Set blocking mode, 0 is non-blocking, 1 "
            "for blocking (Default) \n\n"
            "\t--report-input-metadata  Enable metadata reporting for input "
            "header parsing error\n\n"
            "\t-v4l2-memory-out-plane <num>       Specify memory type to be "
            "used on Output Plane [1 = V4L2_MEMORY_MMAP, 2 = "
            "V4L2_MEMORY_USERPTR], Default = V4L2_MEMORY_MMAP\n\n"
            "\t-v4l2-memory-cap-plane <num>       Specify memory type to be "
            "used on Capture Plane [1 = V4L2_MEMORY_MMAP, 2 = "
            "V4L2_MEMORY_DMABUF], Default = V4L2_MEMORY_DMABUF\n\n"
            "\t-s <loop-count>      Stress test [Default = 1]\n\n"
            "\t-extra_cap_plane_buffer <num>      Specify extra capture plane "
            "buffers (Default=1, MAX=32) to be allocated\n"
#ifndef USE_NVBUF_TRANSFORM_API
            "\t--do-yuv-rescale     Rescale decoded YUV from full range to "
            "limited range\n\n"
#endif
        ;
}

static uint32_t get_decoder_type(char* arg)
{
    if (!strcmp(arg, "H264"))
...
int parse_csv_args(VideoDecodeContext_T* ctx, int argc, char* argv[])
{
    ...
    while ((arg = *(++argp))) {
        if (!strcmp(arg, "-o")) {
            ...
            CSV_PARSE_CHECK_ERROR(ctx->dec_fps <= 0,
                                  "decode fps should be > 0");
            ...
            ;
            CSV_PARSE_CHECK_ERROR((ctx->extra_cap_plane_buffer < 1 ||
                                   ctx->extra_cap_plane_buffer > 32),
                                  "extra capture plane buffer should be "
                                  "greater than 0 & less than 33");
        }
