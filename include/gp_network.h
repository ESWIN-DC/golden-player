...
public:
          ...
          m_qMessagesIn(qIn)
    {
        ...
        if (m_nOwnerType == owner::server) {
            // Connection is Server -> Client, construct random data for the
            // client to transform and send back for validation
    ...
    // This ID is used system wide - its how clients will understand other
    // clients exist across the whole system.
    uint32_t GetID() const { return id; }

public:
    void ConnectToClient(GPlayer::net::server_interface<T>* server,
                         uint32_t uid = 0)
    {
        if (m_nOwnerType == owner::server) {
            if (m_socket.is_open()) {
                ...
                // precisely the validation data sent back from the client
...
public:
    ...
    void Send(const message<T>& msg)
    {
        asio::post(m_asioContext, [this, msg]() {
            ...
            // Either way add the message to the queue to be output. If no
            // messages were available to be written, then start the process of
...
private:
    ...
    void WriteHeader()
    {
        ...
        asio::async_write(m_socket,
                          ...
                          [this](std::error_code ec, std::size_t length) {
                              // asio has now sent the bytes - if there was a
                              // problem an error would be available...
                              if (!ec) {
                                  // ... no error, so check if the message
                                  // header just sent also has a message body...
                                  if (m_qMessagesOut.front().body.size() > 0) {
                                      // ...it does, so issue the task to write
                                      // the body bytes
                                      WriteBody();
                                  }
                                  else {
                                      ...
                                      // If the queue is not empty, there are
                                      // more messages to send, so make this
                                      // happen by issuing the task to send the
                                      // next header.
                                  ...
                                  // ...asio failed to write the message, we
                                  // could analyse why but for now simply assume
                                  // the connection has died by closing the
                                  // socket. When a future attempt to write to
                                  // this client fails due to the closed socket,
    ...
    void WriteBody()
    {
        ...
        asio::async_write(m_socket,
                          ...
                          [this](std::error_code ec, std::size_t length) {
                              if (!ec) {
                                  // Sending was successful, so we are done with
                                  // the message and remove it from the queue
                                  m_qMessagesOut.pop_front();

                                  // If the queue still has messages in it, then
                                  // issue the task to send the next messages'
                                  // header.
                                  if (!m_qMessagesOut.empty()) {
                                      WriteHeader();
                                  }
                                  ...
                                  // Sending failed, see WriteHeader()
                                  // equivalent for description :P
                                  std::cout << "[" << id
                                            << "] Write Body Fail.\n";
                                  m_socket.close();
                              }
    ...
    void ReadHeader()
    {
        // If this function is called, we are expecting asio to wait until it
        // receives enough bytes to form a header of a message. We know the
        // headers are a fixed size, so allocate a transmission buffer large
        // enough to store it. In fact, we will construct the message in a
        // "temporary" message object as it's convenient to work with.
        asio::async_read(
            m_socket,
            asio::buffer(&m_msgTemporaryIn.header, sizeof(message_header<T>)),
            [this](std::error_code ec, std::size_t length) {
                if (!ec) {
                    // A complete message header has been read, check if this
                    // message has a body to follow...
                    if (m_msgTemporaryIn.header.size > 0) {
                        // ...it does, so allocate enough space in the messages'
                        // body vector, and issue asio with the task to read the
                        // body.
                    ...
                    // disconnect has occurred. Close the socket and let the
                    // system tidy it up later.
    ...
    void ReadBody()
    {
        ...
        // header request we read a body, The space for that body has already
        // been allocated in the temporary message object, so just wait for the
        // bytes to arrive...
        asio::async_read(m_socket,
                         ...
                         [this](std::error_code ec, std::size_t length) {
                             if (!ec) {
                                 // ...and they have! The message is now
                                 // complete, so add the whole message to
                                 // incoming queue
                                 AddToIncomingMessageQueue();
                             }
                             else {
    ...
    void WriteValidation()
    {
        asio::async_write(m_socket,
                          ...
                          [this](std::error_code ec, std::size_t length) {
                              if (!ec) {
                                  // Validation data sent, clients should sit
                                  // and wait for a response (or a closure)
                                  if (m_nOwnerType == owner::client)
                                      ReadHeader();
                              }
                              else {
                ...
                if (!ec) {
                    if (m_nOwnerType == owner::server) {
                        ...
                        if (m_nHandshakeIn == m_nHandshakeCheck) {
                            // Client has provided valid solution, so allow it
                            // to connect properly
    ...
    {
        // Shove it in queue, converting it to an "owned message", by
        // initialising with the a shared pointer from this connection object
        if (m_nOwnerType == owner::server)
        ...
        // wil just sit and wait for bytes to arrive, and the message
        // construction process repeats itself. Clever huh?
        ReadHeader();
    }
...
public:
    ...
    void WaitForClientConnection()
    {
        ...
        // This is the purpose of an "acceptor" object. It will provide a unique
        // socket for each incoming connection attempt
        m_asioAcceptor.async_accept([this](std::error_code ec,
            ...
            if (!ec) {
                ...
                if (OnClientConnect(newconn)) {
                    // Connection allowed, so add to container of new
                    // connections
                    m_deqConnections.push_back(std::move(newconn));

                    // And very important! Issue a task to the connection's
                    // asio context to sit and wait for bytes to arrive!
                    m_deqConnections.back()->ConnectToClient(this,
                                                             nIDCounter++);
                    ...
                    // Connection will go out of scope with no pending tasks, so
                    // will get destroyed automagically due to the wonder of
                    // smart pointers
                }
            }
            else {
...
protected:
    ...
    // Order of declaration is important - it is also the order of
    // initialisation
    asio::io_context m_asioContext;
