...
public:
    static ArgusCamera* createArgusCamera(const ArgusCameraConfig& config,
                                          int* info = nullptr);
    ~ArgusCamera();
...
{
    return sizeof(uint8_t) * getNumChannels() * mVideoConverterResolution[0] *
           mVideoConverterResolution[1];
}

ArgusCamera* ArgusCamera::createArgusCamera(const ArgusCameraConfig& config,
                                            int* info)
{
    Argus::Status status;
    ...
    camera->mVideoConverter =
        NvVideoConverter::createVideoConverter("videoConverter");
    ...
    // get camera provider
    auto cameraProvider =
        UniqueObj<CameraProvider>(CameraProvider::create(NULL));
    ...
    camera->mCaptureSession = UniqueObj<CaptureSession>(
        iCameraProvider->createCaptureSession(cameraDevice));
    auto iCaptureSession =
        interface_cast<ICaptureSession>(camera->mCaptureSession);
    if (!iCaptureSession) {
        if (info) {
    ...
    auto streamSettings = UniqueObj<OutputStreamSettings>(
        iCaptureSession->createOutputStreamSettings(NULL));
    auto iOutputStreamSettings =
    ...
    // create stream
    camera->mStream = UniqueObj<OutputStream>(
        iCaptureSession->createOutputStream(streamSettings.get(), &status));
    ...
    camera->mFrameConsumer = UniqueObj<EGLStream::FrameConsumer>(
        EGLStream::FrameConsumer::create(camera->mStream.get()));
    auto iFrameConsumer =
        interface_cast<EGLStream::IFrameConsumer>(camera->mFrameConsumer);
    if (!iFrameConsumer) {
        if (info) {
    ...
    auto iSourceSettings =
        interface_cast<ISourceSettings>(iRequest->getSourceSettings());
    status = iSourceSettings->setSensorMode(
        sensorModes[camera->mConfig.getSensorMode()]);
    if (Argus::STATUS_OK != status) {
        if (info) {
            *info = 18;
        }
    ...
    status = iSourceSettings->setFrameDurationRange(
        Argus::Range<uint64_t>(camera->mConfig.getFrameDurationRange()[0],
    ...
    auto iStreamSettings = interface_cast<IStreamSettings>(
        iRequest->getStreamSettings(camera->mStream.get()));
    if (!iStreamSettings) {
        if (info) {
            *info = 16;
        }
    ...
    status = iStreamSettings->setSourceClipRect(
        Argus::Rectangle<float>(camera->mConfig.getSourceClipRect()[0],
    ...
    camera->mVideoConverter->setOutputPlaneFormat(
        V4L2_PIX_FMT_YUV420M, width, height, V4L2_NV_BUFFER_LAYOUT_PITCH);
    camera->mVideoConverter->setCapturePlaneFormat(
        V4L2_PIX_FMT_ABGR32, width, height, V4L2_NV_BUFFER_LAYOUT_PITCH);

    if (camera->mVideoConverter->output_plane.setupPlane(V4L2_MEMORY_DMABUF, 1,
                                                         false, false) < 0) {
        if (info) {
            *info = 11;
        }
    ...
    if (camera->mVideoConverter->capture_plane.setupPlane(V4L2_MEMORY_MMAP, 1,
                                                          true, false) < 0) {
        if (info) {
            *info = 12;
        }
...
int ArgusCamera::read(uint8_t* data)
{
    ...
    auto iFrameConsumer =
        interface_cast<EGLStream::IFrameConsumer>(mFrameConsumer);
    if (!iFrameConsumer) {
    ...
    auto resolution =
        Argus::Size2D<uint32_t>(mConfig.mVideoConverterResolution[WIDTH_IDX],
                                mConfig.mVideoConverterResolution[HEIGHT_IDX]);
    fd = iNativeBuffer->createNvBuffer(
        resolution, NvBufferColorFormat::NvBufferColorFormat_YUV420,
    ...
    if (mVideoConverter->capture_plane.dqBuffer(v4l2_buf, &nativeCaptureBuffer,
                                                NULL, 1) < 0) {
...
IArgusCamera* IArgusCamera::createArgusCamera(const ArgusCameraConfig& config,
                                              int* info)
{
