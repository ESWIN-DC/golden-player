...
{
    cerr
        << "\nvideo_encode <in-file> <in-width> <in-height> <encoder-type> "
           "<out-file> [OPTIONS]\n\n"
           "Encoder Types:\n"
           "\tH264\n"
           "\tH265\n"
           ...
           "\t--elossless           Enable Lossless encoding [Default = "
           "disabled,"
           ...
           "\t--insert-spspps-idr   Insert SPS PPS at every IDR [Default = "
           "disabled]\n"
           "\t--insert-vui          Insert VUI [Default = disabled]\n"
           "\t--enable-extcolorfmt  Set Extended ColorFormat (Only works with "
           "insert-vui) [Default = disabled]\n"
           "\t--insert-aud          Insert AUD [Default = disabled]\n"
           "\t--alliframes          Enable all I-frame encoding [Default = "
           "disabled]\n"
           "\t-fps <num> <den>      Encoding fps in num/den [Default = "
           "30/1]\n\n"
           "\t-tt <level>           Temporal Tradeoff level [Default = 0]\n"
           "\t-vbs <size>           Virtual buffer size [Default = 0]\n"
           "\t-nrf <num>            Number of reference frames [Default = "
           "1]\n\n"
           "\t-slt <type>           Slice length type (1 = Number of MBs, 2 = "
           "Bytes) [Default = 1]\n"
           "\t-hpt <type>           HW preset type (1 = ultrafast, 2 = fast, 3 "
           "= medium,  4 = slow)\n"
           "\t-slen <length>        Slice length [Default = 0]\n"
           "\t-sir <interval>       Slice intrarefresh interval [Default = "
           "0]\n\n"
           "\t-nbf <num>            Number of B frames [Default = 0]\n\n"
           "\t-rpc <string>         Change configurable parameters at "
           "runtime\n\n"
           "\t-goldcrc <string>     GOLD CRC\n\n"
           "\t--rcrc                Reconstructed surface CRC\n\n"
           "\t-rl <cordinate>       Reconstructed surface Left cordinate "
           "[Default = 0]\n\n"
           "\t-rt <cordinate>       Reconstructed surface Top cordinate "
           "[Default = 0]\n\n"
           "\t-rw <val>             Reconstructed surface width\n\n"
           "\t-rh <val>             Reconstructed surface height\n\n"
           "\t-rcrcf <reconref_file_path> Specify recon crc reference param "
           "file\n\n"
           "\t--report-metadata     Print encoder output metadata\n"
           "\t--blocking-mode <val> Set blocking mode, 0 is non-blocking, 1 "
           "for blocking (Default) \n\n"
           "\t--input-metadata      Enable encoder input metadata\n"
           "\t--copy-timestamp <st> Enable copy timestamp with start "
           "timestamp(st) in seconds\n"
           "\t--mvdump              Dump encoded motion vectors\n\n"
           "\t--eroi                Enable ROI [Default = disabled]\n\n"
           "\t-roi <roi_file_path>  Specify roi param file\n\n"
           "\t--erps                Enable External RPS [Default = "
           "disabled]\n\n"
           "\t--egdr                Enable GDR [Default = disabled]\n\n"
           "\t--gif                 Enable Gaps in FrameNum [Default = "
           "disabled]\n\n"
           ...
           "\t--erh                 Enable External picture RC [Default = "
           "disabled]\n\n"
           "\t-mem_type_oplane <num> Specify memory type for the output plane "
           "to be used [1 = V4L2_MEMORY_MMAP, 2 = V4L2_MEMORY_USERPTR, 3 = "
           "V4L2_MEMORY_DMABUF]\n\n"
           "\t-gdrf <gdr_file_path> Specify GDR Parameters filename \n\n"
           "\t-gdrof <gdr_out_file_path> Specify GDR Out filename \n\n"
           "\t-smq <max_qp_value>   Max QP per session when external picture "
           "RC enabled\n\n"
           "\t-hf <hint_file_path>  Specify external rate control param "
           "file\n\n"
           ...
           "NOTE: roi parameters need to be feed per frame in following "
           "format\n"
           ...
           "      1   -5   40  40  40  40\n"
           "      3   -4   34  34  16  16  -5  70  70  18  18  -3  100  100  "
           "34  34\n"
           ...
           "\"f<frame_num1>,<prop_id1><val>,<prop_id2><val>#f<frame_num1>,<"
           "prop_id1><val>,<prop_id2><val>#...\"\n"
           ...
           "\tr<num/den>  Framerate\n"
           "\ti1          Force I-frame\n\n"
           "NOTE: These encoding parameters are slightly imprecisely updated "
           "depending upon the number of\n"
           "frames in queue and/or processed already.\n";
}

static uint32_t get_encoder_type(char* arg)
{
    if (!strcmp(arg, "H264"))
...
static int32_t get_encoder_ratecontrol(char* arg)
{
    if (!strcmp(arg, "cbr"))
...
static int32_t get_encoder_profile_h264(char* arg)
{
...
static int32_t get_encoder_profile_h265(char* arg)
{
    if (!strcmp(arg, "main"))
...
static int32_t get_encoder_level(char* arg)
{
    if (!strcmp(arg, "1.0"))
...
static int32_t get_dbg_level(char* arg)
{
    int32_t log_level = atoi(arg);

    if (log_level < 0) {
        cout << "Warning: invalid log level input, defaulting to setting 0"
             << endl;
        return 0;
    }

    if (log_level > 3) {
        cout << "Warning: invalid log level input, defaulting to setting 3"
             << endl;
        return 3;
    }
...
int parse_csv_args(VideoEncodeContext_T* ctx, int argc, char* argv[])
{
    ...
    while ((arg = *(++argp))) {
        if (!strcmp(arg, "-h") || !strcmp(arg, "--help")) {
            ...
            CSV_PARSE_CHECK_ERROR(ctx->peak_bitrate == 0,
            ...
            CSV_PARSE_CHECK_ERROR(
                intval == -1, "Unsupported value for ratecontrol: " << *argp);
            ...
            switch (atoi(*argp)) {
                ...
                default:
                    CSV_PARSE_CHECK_ERROR(
                        true, "Unsupported value for encoder HW Preset Type: "
                                  << *argp);
            }
        }
            ...
            switch (num) {
                case 1:
                    ...
                    break;
                case 2:
                    ctx->output_memory_type = V4L2_MEMORY_USERPTR;
                    break;
                case 3:
            ...
            CSV_PARSE_CHECK_ERROR(ctx->nMinQpI > 51,
                                  "Min Qp should be >= 0 and <= 51");
        }
        else if (!strcmp(arg, "-MaxQpI")) {
            argp++;
            CHECK_OPTION_VALUE(argp);
            ctx->nMaxQpI = atoi(*argp);
            CSV_PARSE_CHECK_ERROR(ctx->nMaxQpI > 51,
                                  "Max Qp should be >= 0 and <= 51");
        }
        else if (!strcmp(arg, "-MinQpP")) {
            argp++;
            CHECK_OPTION_VALUE(argp);
            ctx->nMinQpP = atoi(*argp);
            CSV_PARSE_CHECK_ERROR(ctx->nMinQpP > 51,
                                  "Min Qp should be >= 0 and <= 51");
        }
            ...
            CSV_PARSE_CHECK_ERROR(ctx->nMaxQpP > 51,
                                  "Max Qp should be >= 0 and <= 51");
        }
            ...
            CSV_PARSE_CHECK_ERROR(ctx->nMinQpB > 51,
                                  "Min Qp should be >= 0 and <= 51");
        }
            ...
            CSV_PARSE_CHECK_ERROR(ctx->nMaxQpB > 51,
                                  "Max Qp should be >= 0 and <= 51");
        }
            ...
            CSV_PARSE_CHECK_ERROR(
                ctx->rl < 0,
                "Reconstructed surface Left cordinate should be >= 0");
        }
            ...
            CSV_PARSE_CHECK_ERROR(
                ctx->rt < 0,
                "Reconstructed surface Top cordinate should be >= 0");
        }
        else if (!strcmp(arg, "-rw")) {
            argp++;
            CHECK_OPTION_VALUE(argp);
            ctx->rw = atoi(*argp);
            CSV_PARSE_CHECK_ERROR(ctx->rw <= 0,
                                  "Reconstructed surface width should be > 0");
        }
        else if (!strcmp(arg, "-rh")) {
            argp++;
            CHECK_OPTION_VALUE(argp);
            ctx->rh = atoi(*argp);
            CSV_PARSE_CHECK_ERROR(ctx->rh <= 0,
                                  "Reconstructed surface height should be > 0");
        }
            ...
            CSV_PARSE_CHECK_ERROR(ctx->start_ts < 0,
                                  "start timestamp should be >= 0");
